...
def test_available_buns_elements_type():
    buns = Database().available_buns()
    assert all(isinstance(bun, Bun) for bun in buns)


def test_available_buns_contains_expected_names():
    buns = Database().available_buns()
...
def test_available_ingredients_elements_type():
    ingredients = Database().available_ingredients()
    assert all(isinstance(ing, Ingredient) for ing in ingredients)


def test_available_ingredients_contains_expected_names():
    ingredients = Database().available_ingredients()
...
def test_bun_get_name_and_price():
    bun = Bun("black bun", 100)
    assert bun.get_name() == "black bun"
    assert bun.get_price() == 100


def test_ingredient_getters():
...
def test_database_initialization_with_mocks(mock_bun_class, mock_ingredient_class):
...
def test_get_price_calculation_with_mocks():
    mock_bun = MagicMock()
